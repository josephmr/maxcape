...
export function groupEventsByDay(events: RawEvent[]): DayGroup[] {
	const dayMap = new Map<string, { skills: Map<string, { min: number; max: number }>; items: Set<string> }>();
		...
		if (event.eventType === 'SKILL_LEVEL_UP') {
			...
			if (skill && level > 0) {
				...
				if (!existing) {
					...
					bucket.skills.set(skill, { min: Math.min(existing.min, level), max: Math.max(existing.max, level) });
				}
			}
