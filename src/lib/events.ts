...
export type DayGroup   = EventGroup;
export type MonthGroup = EventGroup;
...
export function groupEventsByMonth(events: RawEvent[]): MonthGroup[] {
	...
	return Array.from(monthMap.keys())
		...
		.map((monthKey) => {
			...
			const dateLabel = new Date(Date.UTC(y, m - 1, 1)).toLocaleDateString('en-GB', {
				month: 'long',
				year: 'numeric',
				timeZone: 'UTC'
			});
