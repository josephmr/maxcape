<script lang="ts">
	...
	let { skill, level, levelsGained }: { skill: string; level: number; levelsGained: number } =
		$props();
</script>
...
<div class="flex items-stretch border border-osrs-chip-border overflow-hidden cursor-default">
	...
	>
		<img
			...
			alt={skill}
			class="pixelated w-5 h-5 lg:w-6 lg:h-6 object-contain"
		/>
	</div>
	<div class="bg-osrs-chip-bg chip-content-pad flex flex-col justify-center">
		...
		<span class="font-pixel text-[10px] lg:text-[13px] text-osrs-gold-bright leading-[1.4]">
			{level}{#if levelsGained > 1}<span class="text-[8px] lg:text-[10px] text-osrs-gold-dim ml-1"
					>+{levelsGained}</span
				>{/if}
		</span>
	</div>
